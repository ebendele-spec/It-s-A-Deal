...
| `manifest.webmanifest` | Makes the site installable as a home-screen app | Yes |
| `sw.js` | Service worker: offline support + install eligibility | Yes |
| `icons/` | App icons + favicon (deal sheet with gold check seal) | Yes |
...
- **Installable app (PWA)**: on phones, a dismissible banner invites salespeople
  to add It's a Deal to their home screen — Android gets a real Install button,
  iOS shows Share → "Add to Home Screen" instructions. Once installed it opens
  full-screen with the app icon, and the service worker keeps the form loading
  even with no signal on the lot. Deploy `manifest.webmanifest`, `sw.js`, and
  the `icons/` folder alongside index.html (same paths).
- **VIN Photo button** (next to Last 8 of VIN, all devices): photo of the VIN
  plate/sticker, compressed like ID photos, no cropping, attached as its own
  labeled page to both printed and emailed PDFs.
- **Design refresh**: polished web UI (gold section ticks, refined masthead,
  softer shadows) and PDF (MHS monogram seal, deal-type pills, branded footer
